...
struct minmaxstack
{
    ...
    int min() { return smin.back(); }
    int max() { return smax.back(); }
...
void solve()
{
    ...
    forn(i, d) { cout << i + 1 << " " << i + 2 << endl; }
    n -= d + 1;
    l -= 2;
    //   cout << "MAIN PATH PRINTED" << endl;
    // checks if there are not more leaves than nodes

    int eachbranchlength = n / l;
    int numberofotherbranches = n % l;
    int numberofnormalbranches = l - numberofotherbranches;
    int currnode = d + 2;
    //   bug(eachbranchlength, numberofnormalbranches, numberofotherbranches,
    //       currnode);
    for (int i = 2; i <= d + 1; i++)
    {

        if ((i - 1) + (eachbranchlength) <= d and (d - i) + eachbranchlength <= d)
        {
            while (numberofnormalbranches--)
            {

                int node1 = i;
                int node2 = currnode;
                forn(i, eachbranchlength)
                {
        ...
        if ((i - 1) + (eachbranchlength + 1) <= d and
            (d - i) + eachbranchlength + 1 <= d)
        {
            while (numberofotherbranches--)
            {
                int node1 = i;
                int node2 = currnode;
                forn(i, eachbranchlength + 1)
                {
                    cout << node1 << " " << node2 << endl;
                    node1 = node2;
                    currnode++;
...
}
{
    ...
    for (ll i = 0; i < n - 1; i++)
    {
        if (v[i] == v[i + 1] && v[n - i - 1] != v[i])
        {
            if (i == 0 || (2 * i) == n || (v[i - 1] != v[n - i - 1] && v[n - i - 2] != v[i]))
            {
                // cout << "n" << endl;
                swap(v[i], v[n - i - 1]);
            }
