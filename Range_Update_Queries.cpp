...
void build(int ind, int low, int high, vecll &v)
{
    if (low == high)
    {
        seg[ind] = v[low];
...
int query(int ind, int l, int r, int lo, int hi)
{
    if (lazy[ind] != 0)
    {
        seg[ind] += (hi - lo + 1) * lazy[ind];
    ...
    int mid = (lo + hi) >> 1;
    int left = query(2 * ind + 1, l, r, lo, mid);
    int right = query(2 * ind + 2, l, r, mid + 1, hi);
    return left + right;
}
void solve()
{
    ...
    while (q--)
    {
        ...
        if (x == 1)
        {
            ...
            l--;
            r--;
            lazyUpdate(0, 0, n - 1, l, r, u);
        }
        else
        {
            di(k);
            k--;
            cout << query(0, k, k, 0, n - 1) << endl;
        }
    }
}
signed main()
